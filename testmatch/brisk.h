...
struct BriskLongPair{
	...
	public:
		...
		const static int normType = NORM_L2;
		...
		//		(const __m128i*)(b),
		//		size/16);
			int lut[256] = { 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4 };
			int result = 0;
			for( int i = 0; i<size; i++ ){
				unsigned char xor_res = a[i] ^ b[i];
				unsigned char upper = xor_res >> 4;
				unsigned char lower = xor_res & 0x0f;
				result += lut[upper] + lut[lower];
			}
			printf("%d\n",result);
			return result;
	    }
	...
	public:
		...
		void getAgastPoints(uint8_t threshold, std::vector<cv::KeyPoint>& keypoints);
	...
	class CV_EXPORTS ZKBriskDescriptorExtractor : public cv::BriskDescriptorExtractor{
	public:
		// create a descriptor with standard pattern
		ZKBriskDescriptorExtractor(int n_rings=5, int n_points_per_ring=16, double patternScale=1.0);
		virtual ~ZKBriskDescriptorExtractor();
	...
	protected:
		...
		__inline__ void rotate512bit( unsigned int* des, unsigned int* src, int theta ) const;

	};

	class CV_EXPORTS_W ZKMatcher : public cv::DescriptorMatcher
	{
	public:
		CV_WRAP ZKMatcher();
		virtual ~ZKMatcher() {}
